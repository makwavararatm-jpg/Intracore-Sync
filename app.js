...
onValue(vouchersRef, (snapshot) => { 
    ...
    vouchersArray.slice(0, 10).forEach(v => { 
        ...
        // Pass the extra data (Price, Uptime, Data) to the printReceipt function
        tbody.innerHTML += `<tr><td style="font-family: monospace; font-size: 1.1rem; font-weight: 600; color: #111827;">${v.code}</td><td style="font-weight:500;">${v.label}</td><td>${deliveryBadge}</td><td><span class="badge-active-small">${v.status.toUpperCase()}</span></td><td style="color:#6b7280; font-size:0.8rem;">${dateStr}</td><td><button class="btn-print" onclick="printReceipt('${v.code}', '${v.label}', ${v.price || 0}, '${v.uptimeLimit || 'Unlimited'}', '${v.dataLimit || 'Unlimited'}', '${dateStr}')">🖨️ Print</button></td></tr>`; 
    }); 
});
